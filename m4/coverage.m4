...
if test "x$enable_coverage" = "xyes"; then
  ...
  if test "x$LCOV" = "x"; then
    AC_MSG_ERROR([cannot find lcov in your path, but it is required. Please install lcov.])
  fi
  if test "x$GENHTML" = "x"; then
    AC_MSG_ERROR([cannot find genhtml in your path, but it is required. Please install lcov.])
  fi
